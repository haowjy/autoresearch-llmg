...
This skill applies when work coordination is warranted — when a task needs work-scoped artifacts, lifecycle tracking, or selected-work semantics. Not every spawn needs a work item; small direct tasks can run without one.

When coordination is warranted, ensure your session is attached to the correct work item before spawning. Spawns inherit the attachment and get `$MERIDIAN_ACTIVE_WORK_DIR` set automatically.
