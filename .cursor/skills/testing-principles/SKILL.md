...
description: |
  Load when deciding what kind of test a change needs, when a test suite feels off, or when routing work to @smoke-tester or @coder with testing skills. Shared foundation — tier selection, functional core / imperative shell, and the tradeoffs behind each tier. Always include this skill when spawning @coder for testing work.
