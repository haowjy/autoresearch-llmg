"""Shared P0-TW matrix runner (Wave A harness, Wave B agent, Wave C smoke)."""
...
from llmg.data.temporalwiki import cl_query_from_row
...
def _run_harness_cell(
    ...
    *,
    ...
    questions, gold_subjects, _, as_of = eval_rows_fn(
...
def _run_agent_cell(
    ...
    *,
    ...
    questions, gold_subjects, gold_answers, as_of = eval_rows_fn(
        ds, cell["eval_split"], max_rows=max_rows
    )
...
def run_matrix(
    *,
    load_dataset_fn: Callable[[], DatasetDict],
    eval_rows_fn: EvalRowsFn = eval_rows_easy,
    dev_run_dir_name: str,
    k: int = 5,
    ...
    run_dir = session.run_dir if session else Path(f"llmg/runs/{dev_run_dir_name}")
    ...
    if active_waves is None:
        if run_phase == "official":
            active_waves = official_waves if official_waves is not None else ["B"]
        else:
            active_waves = ["A", "C"]
